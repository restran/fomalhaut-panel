...
function appCtrl($scope, $http) {
    ...
    $scope.model = {old_password: '', new_password: ''};
    ...
    function doSubmit() {
        if (weekPasswordDict[$scope.model.new_password] != undefined) {
            toastr["error"]('您的密码太弱存在被破解的风险');
            return;
        }

        if (!(/[0-9]+/.test($scope.model.new_password) &&
            /[a-zA-Z]+/.test($scope.model.new_password))) {
        ...
        var shaObj = new jsSHA(postData['old_password'], "TEXT");
        postData['old_password'] = shaObj.getHash("SHA-1", "HEX");
        shaObj = new jsSHA(postData['new_password'], "TEXT");
